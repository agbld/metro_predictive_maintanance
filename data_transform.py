...
def get_X_of_the_date(date: date, device_keys_table, event_keys_table, time_window=7, time_steps=4, host=mssql_host, user=mssql_user, passward=mssql_password, database=mssql_database):
  ...
  for i in range(1, time_steps + 1):
    ...
    """.format(i, str(date - timedelta(days=i * time_window)), str(date + timedelta(days=1) - timedelta(days=(i - 1) * time_window)), cmd_key_constrain, atim_event_alarm_table)  
  ...
  for i in range(1, time_steps + 1):
    ...
    """.format(i, str(date - timedelta(days=i * time_window)), str(date + timedelta(days=1) - timedelta(days=(i - 1) * time_window)), malfunc_report_table)
    if i < time_steps: cmd += " union "
...
def get_Y_of_the_date(date: date, device_keys_table, time_window=7, host=mssql_host, user=mssql_user, passward=mssql_password, database=mssql_database):
  ...
  group by concat(svce_loc_id, dev_name)
  """.format(str(date + timedelta(days=1)), str(date + timedelta(days=time_window + 1)), cmd_key_constrain, malfunc_report_table)
  cursor.execute(cmd)  
  for row in cursor:
    # query_counts += 1
